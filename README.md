...
- **필요성**
  - 모바일 플랫폼에서 사용자가 언제 어디서나 쉽게 접근할 수 있는 멀티플레이 게임에 대한 수요 증가
  - 다양한 미니게임을 하나의 애플리케이션에서 자유롭게 선택하고 플레이할 수 있는 환경이 필요함
  - 실시간 상호작용을 통해 사용자 간의 몰입감 있는 게임 경험을 제공함으로써 유저 만족도 향상
...
- **Photon Engine의 최적화 및 확장성**
  - **Photon Unity Networking (PUN)**: PUN은 Unity를 위한 Photon의 네트워크 솔루션으로 멀티플레이어 게임에서 서버와 클라이언트 간 실시간 통신을 쉽게 구현할 수 있도록 지원함 이는 실시간 데이터 송수신과 동기화 기능을 제공해 게임 상태와 이벤트를 빠르고 정확하게 전달하여 멀티플레이어 환경에서 사용자 경험을 최적화함
  - PUN을 사용하여 멀티플레이어 게임 환경을 최적화하고 추후에 게임 콘텐츠를 확장할 수 있도록 시스템을 구성해야 함
  - Photon 서버를 통해 사용자 간 원활한 통신과 동기화가 가능하며 서버 부하 관리와 확장성을 고려한 설계가 필요함
...
- **사용 기술 및 선정 이유**
  ...
  - **Photon Engine**: PUN을 통해 서버와 클라이언트 간의 통신 및 멀티플레이 기능 구현
...
- **기술적 성과와 배운 점**
  ...
  - 네트워크 불안정 상황에서의 자동 재연결 및 오류 복원 기능을 통해 안정적인 네트워크 환경 유지의 중요성을 학습함

- **향후 개선 및 확장 방안**
  - 새로운 미니게임과 게임 모드 추가를 통해 사용자 경험을 풍부하게 하고자 하며 안정적인 서버 관리를 통해 원활한 멀티플레이 환경을 제공할 계획
